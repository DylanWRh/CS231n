    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            ...
            if self.cell_type == 'rnn':
                ...
                h,prev_c,_ = lstm_step_forward(word_embed,h,prev_c,Wx,Wh,b)
            
            hh = np.array([h]).transpose((1,0,2))
            next_word_embed,_ = temporal_affine_forward(hh,W_vocab,b_vocab)
            next_word_embed = next_word_embed[ : ,0, : ]
            
            word = np.argmax(next_word_embed,axis=1)
            captions[ : ,i] = word
            word=np.array([word]).T
