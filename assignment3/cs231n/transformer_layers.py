...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        m = torch.arange(max_len).reshape((max_len,1)) * (10000**(-torch.arange(0,embed_dim,2)/embed_dim)).reshape((1,embed_dim//2))
        pe[ : , : , : :2] = torch.sin(m)
        pe[ : , : ,1: :2] = torch.cos(m)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.num_heads
        Q = self.query(query).reshape((N,S,H,D//H)).transpose(1,2)
        K = self.key(key).reshape((N,T,H,D//H)).transpose(1,2)
        V = self.value(value).reshape((N,T,H,D//H)).transpose(1,2)
        
        MM = Q.matmul(K.transpose(2,3))/torch.sqrt(torch.Tensor([D/H]))
        if attn_mask is not None:
            MM = MM.masked_fill(attn_mask==0,float('-inf'))
        
        softmax = nn.Softmax(dim=3)
        NN = softmax(MM)
        
        NN = self.dropout(NN)
        PP = NN.matmul(V).transpose(1,2).reshape(N,S,D)
