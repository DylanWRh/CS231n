...
def rnn_step_backward(dnext_h, cache):
    ...
    dbefore_tanh = dnext_h*(1-next_h*next_h)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h[0],cache_cur = rnn_step_forward(x[0],h0,Wx,Wh,b)
    
    for i in range(1,T):
        h[i],cache_cur = rnn_step_forward(x[i],h[i-1],Wx,Wh,b)
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1,-1,-1):
        cache_ = (x[i],h[i-1],Wx,Wh,h[i]) if i!=0 else (x[i],h0,Wx,Wh,h[i])
        dx_,dprev_h_,dWx_,dWh_,db_ = rnn_step_backward(dh[i]+dh0,cache_)
        dx[i] = dx_
        dh0 = dprev_h_
        dWx += dWx_
        dWh += dWh_
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dprev_c,df,di,dg = dnext_c * f,dnext_c * prev_c,dnext_c * g,dnext_c * i
